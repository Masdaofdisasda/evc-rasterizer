...
if a ~= 0
    ...
    const1 = 1/lineEq(A1,B1,C1,x1,y1);
    const2 = 1/lineEq(A2,B2,C2,x2,y2);
    const3 = 1/lineEq(A3,B3,C3,x3,y3);
    ...
    for i = xmin:xmax
        for j = ymin:ymax
            O1 = lineEq(A1,B1,C1,i,j);
            O2 = lineEq(A2,B2,C2,i,j);
            O3 = lineEq(A3,B3,C3,i,j);
            if (O1 <= 0 && O2 <= 0 && O3 <= 0)
                alpha = O1*const1;
                beta = O2*const2;
                gamma = O3*const3;
                ...
                framebuffer.setPixel(round(i), round(j), depth, color);
            end
        end
    end
